...
// 基础限制常量定义
#define MAX_NAME_LEN    32      // 名字最大长�?
#define TEAM_SIZE       3       // 每个队伍的角色数�?
#define MAX_HAND_SIZE   10      // 最大手牌数�?
#define MAX_DECK_SIZE   30      // 卡组最大容�?
#define MAX_TURN_ACTIONS 16     // 每名玩家单轮最多记录的操作�?
...
typedef struct {
    ...
    ElementType element;        // 元素属�?
    CardType type;              // 卡牌类型
    int energy_cost;            // 能量消�?
    int base_damage;            // 基础伤害
    int base_defense;           // 基础防御/护盾�?
    int base_heal;              // 基础治疗�?
    BuffType buff_effect;       // 施加的Buff类型
    int buff_value;             // Buff数�?
    int buff_duration;          // Buff持续回合�?
    TargetType target_type;     // 卡牌目标类型（敌方单�?全体、己方单�?全体�?
} Card;
...
typedef struct {
    ...
    ElementType element;        // 角色元素属�?
    int hp;                     // 当前生命�?
    int max_hp;                 // 最大生命�?
    int speed;                  // 速度（决定行动顺序）
    int is_alive;               // 是否存活状态标�?
    int buffs[BUFF_COUNT];      // 当前持有的每种Buff的层�?持续时间
} Character;

// 玩家/队伍结构体定�?
typedef struct {
    int player_id;              // 玩家ID
    char name[MAX_NAME_LEN];    // 玩家�?
    Character team[TEAM_SIZE];  // 玩家的角色队�?
    int active_idx;             // 当前出战角色的索�?
    int energy;                 // 当前能量�?
    int max_energy;             // 最大能量值上�?
    Card hand[MAX_HAND_SIZE];   // 手牌列表
    int hand_count;             // 当前手牌数量
    Card draw_pile[MAX_DECK_SIZE];   // 抽牌�?
    int draw_count;                  // 抽牌堆数�?
    Card discard_pile[MAX_DECK_SIZE];// 弃牌�?
    int discard_count;               // 弃牌堆数�?
} Player;
...
typedef struct {
    ...
    int current_turn;           // 当前轮到哪位玩家行动的出牌权�?�?�?
    int game_stage;             // 游戏阶段标志
    AppScene current_scene;     // 当前全局游戏状�?
    int global_auras[5];        // 全局特征/环境干涉光环
} GameState;

// 行动指令结构体定�?
typedef struct {
    ActionType type;            // 执行的行动类�?
    int actor_id;               // 行动发起者ID
    int card_hand_idx;          // 当打出卡牌时使用，记录出手牌列表索引
    int switch_to_idx;          // 当切换角色时使用，记录目标角色索�?
